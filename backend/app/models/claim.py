...
from sqlalchemy import String, Text, SmallInteger, Integer, ForeignKey, JSON, Boolean
...
class Claim(Base):
    ...
    # uploaded | processing | analyzed | needs_review | approved
    ...
    uploaded_by: Mapped["User | None"] = relationship(back_populates="claims")
    ...
    activities: Mapped[list["ClaimActivity"]] = relationship(back_populates="claim", cascade="all, delete-orphan")
...
class ClaimFix(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(Uuid(as_uuid=True), primary_key=True, default=uuid.uuid4)
    claim_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("claims.id", ondelete="CASCADE"))
    issue_id: Mapped[uuid.UUID | None] = mapped_column(ForeignKey("claim_issues.id", ondelete="SET NULL"), nullable=True)
    created_at: Mapped[datetime] = mapped_column(default=utcnow)
